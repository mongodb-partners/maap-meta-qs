...
# Variables
INFRA_STACK_NAME="MAAP-Meta-Stack-Infrastructure"
...
ServerInstanceProfile="MAAPMetaServerInstanceProfile"

VolumeSize=100 #Disk Storage Space

GIT_REPO_URL="https://github.com/mongodb-partners/maap-meta-qs"
...
LLM_MODEL_ID="us.meta.llama3-3-70b-instruct-v1:0" #AI Memory LLM Model ID
EMBEDDING_MODEL_ID="amazon.titan-embed-text-v1" #Semantic Cache Embedding Model ID

# Agent Model IDs for AWS Bedrock
...
deploy_ec2()
{
  ...
  log_message "$(aws cloudformation create-stack \
    ...
    --parameters \
      ...
      ParameterKey=AIMemoryReplicas,ParameterValue=$AIMemoryReplicas \
      ParameterKey=SemanticCacheReplicas,ParameterValue=$SemanticCacheReplicas \
      ...
      ParameterKey=ReflectionAgent,ParameterValue="$REFLECTION_AGENT" \
      ParameterKey=SolutionAgent,ParameterValue="$SOLUTION_AGENT" \
      ParameterKey=InquiryAgent,ParameterValue="$INQUIRY_AGENT" \
      ParameterKey=GuidanceAgent,ParameterValue="$GUIDANCE_AGENT" \
      ParameterKey=VisualAgent,ParameterValue="$VISUAL_AGENT" \
      ParameterKey=CodingAgent,ParameterValue="$CODING_AGENT" \
      ParameterKey=AnalyticsAgent,ParameterValue="$ANALYTICS_AGENT" \
      ParameterKey=ReasoningAgent,ParameterValue="$REASONING_AGENT" \
